class ApplicationInteraction < ActiveInteraction::Base
  ...
  def compose(other, *args)
    ...
    if outcome.invalid?
      ...
      errors_new = ActiveInteraction::Errors.new(outcome.errors.messages.keys.map { |key| "#{class_name.downcase}_#{key}".parameterize.split('-').join('_') })
      outcome.errors.messages.each do |k,v|
        errors_new.add("#{class_name.downcase}_#{k}".parameterize.split('-').join('_'), v.first)
      end
