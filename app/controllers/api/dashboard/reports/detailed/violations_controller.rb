    ...
    module Reports
      module Detailed
        class ViolationsController < ApplicationController
          api :GET, '/api/dashboard/reports/detailed/violations', 'Violation detailed report'
          header :Authorization, 'Auth token', required: true
          param :violation_status, String, 'Violatios status'
          ...
          def index
            if params[:violation_status] == 'rejected'
              payload = ::Reports::Detailed::RejectedViolations.run(params.merge(current_user: current_user))
            else
              payload = ::Reports::Detailed::Violations.run(params.merge(current_user: current_user))
            end
