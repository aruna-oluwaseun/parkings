  ...
  module Dashboard
    class AdminsQuery < ::ApplicationQuery
      def call
        ...
        scope = Admin.where(id: user.managed_users.pluck(:id))
        scope = scope.joins(:role).where(roles: { display_name: role_names }).select('admins.*, roles.display_name') if role_names.present?
