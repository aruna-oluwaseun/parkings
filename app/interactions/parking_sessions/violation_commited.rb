module ParkingSessions
  class ViolationCommited < BaseEvent
    ...
    def create_ticket
      transactional_create!(Parking::Ticket, violation: object, agency: parking_lot.agency, admin: parking_lot.parking_admin)
    end
    ...
    def notify_admin
      ...
      ViolationMailer.commited(email, object).deliver_later
    end

    def notify_manager
      email = parking_lot.agency&.manager&.email
      ViolationMailer.commited(email, object).deliver_later unless email.blank?
    end

    def notify_officer
      email = object.rule.officer&.email
      ViolationMailer.commited(email, object).deliver_later unless email.blank?
    end
    ...
    def notify_user
      ...
      UserNotifier.violation_commited(vehicle.user, session, vehicle.user.email, object)
    end

    def notify_recipients
      recipients = object.rule.recipients
