...
RSpec.describe Api::Dashboard::ParkingSlotsController, type: :request do
  ...
  context 'success' do
    subject do
      get "/api/dashboard/parking_slots/#{parking_slot.id}/sessions", headers: { Authorization: get_auth_token(admin) }
    end
  ...
  context 'fail' do
    context 'unauthorized' do
      subject do
        get "/api/dashboard/parking_slots/#{parking_slot.id}/sessions"
      end
    ...
    context 'when parking slot does not exist' do
      subject do
        get '/api/dashboard/parking_slots/9999/sessions', headers: { Authorization: get_auth_token(admin) }
      end
