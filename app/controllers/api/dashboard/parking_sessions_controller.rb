module Api
  module Dashboard
    class ParkingSessionsController < ::Api::Dashboard::ApplicationController
      ...
      api :GET, '/api/dashboard/parking_session/:id/logs', 'Fetch parking session log details'
      header :Authorization, 'Auth token', required: true
      ...
      param :range, Hash, 'When the session log was created', required: false do
        param :from, String, 'Date formatted %Y-%m-%d', required: false
        param :to, String, 'Date formatted %Y-%m-%d', required: false
      end
      ...
      def logs
        ...
        authorize! parking_session
        scope = paginate ParkingSessionLogQuery.call(params.merge(user: current_user, parking_session: parking_session))
        respond_with scope, each_serializer: ::Api::Dashboard::Parking::SessionLogSerializer
      end
    end
  end
end
