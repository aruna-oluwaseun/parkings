      ...
      context 'With date range' do
        context 'only with FROM key' do
          ...
          it "should show only payment made on #{DateTime.now + 2.days}" do
            without_timestamping_of Payment do
              parking_sessions_future.each do |ps|
                create(:payment, parking_session: ps, created_at: DateTime.now + 2.days, updated_at: Time.now)
              end
            end
