    ...
    class NotificationsController < ::Api::V1::ApplicationController
      ...
      def mark_read
        User::Notification.transaction do
          User::Notification.where(id: params[:notification_ids]).unread.update_all(status: :read)
        end
      ...
      api :PUT, '/api/v1/notifications/mark_all', 'Mark all notifications as read.'
      header :Authorization, 'Auth token from users#sign_in', required: true

      def mark_all_as_read
        User::Notification.transaction do
          current_user.notifications.where.not(status: :read).each do |notification|
