  ...
  describe 'GET #index' do
    ...
    context 'success: filter by role_name' do
      ...
      context 'with one role name' do
        subject do
          get '/api/dashboard/admins/search', headers: { Authorization: get_auth_token(admin) }, params: {
            role_names: officer_role.display_name
          }
        end
      ...
      context 'with 2 role names' do
        subject do
          get '/api/dashboard/admins/search', headers: { Authorization: get_auth_token(admin) }, params: {
            per_page: 40,
            role_names: [officer_role.display_name, manager_role.display_name]
          }
        end
    ...
    context 'success: manager only see its officers' do
      subject do
      ...
      let!(:agencies) { create_list(:agency, 10, :with_officer, managers: [manager]) }
