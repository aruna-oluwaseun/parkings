    ...
    module Ksk
      class ParkingSessionsController < ::Api::V1::Ksk::ApplicationController
        ...
        def index
          ...
          return session_with_ticket!(:exceeding_grace_period) if ticket_approved?
        ...
        api :PUT, '/api/v1/ksk/parking_sessions/:id/lpn', 'Update LPN based on a parking session'
        header :Authorization, 'Auth token for Kiosk module', required: true
        param :lpn, Integer, 'Expected check out time in seconds'

        def update_lpn
          return session_not_found! unless @session
          result = ParkingSessions::UpdateLpn.run(params.merge(object: @session))
          respond_with result, serializer: serializer
        end
        ...
        def ticket_approved?
          ...
          return violation.ticket.approved?
        end
      end
    end
  end
