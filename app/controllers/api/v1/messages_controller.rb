module Api
  module V1
    class MessagesController < ::Api::V1::ApplicationController
      ...
      api :DELETE, '/api/v1/messages/delete', 'Delete messages'
      param :message_ids, Array, of: Integer, desc: 'Messages ids that should be deleted', required: true
      header :Authorization, 'Auth token from users#sign_in', required: true

      def destroy
        Message.transaction do
          Message.where(id: params[:message_ids]).destroy_all
        end
