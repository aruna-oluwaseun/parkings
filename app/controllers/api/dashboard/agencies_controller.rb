module Api
  module Dashboard
    class AgenciesController < ::Api::Dashboard::ApplicationController
      wrap_parameters :agency, include: Agency.attribute_names + %i[location manager_id officer_ids]
      ...
      param :id, Integer, 'Filter by agency ID', required: false
      param :status, Agency.statuses.keys, required: false
      param :query, String, 'Query (can be substring of name, full address, email, phone or admins/managers name)', required: false
      ...
      param :agency, Hash do
        ...
        param :parking_lot_ids, Array, 'Array of parking lot IDs', required: false
        param :agency_type_id, Integer, 'AgencyType ID', required: true
      ...
      api :DELETE, '/api/dashboard/agencies/:id', 'Delete agency'
      param :id, Integer, 'Agency id', required: true
      header :Authorization, 'Auth token', required: true
      ...
      def destroy
        agency = Agency.find(params[:id])
        authorize! agency
        result = ::Agencies::Delete.run(agency: agency)
        respond_with result
      end
    end
  end
