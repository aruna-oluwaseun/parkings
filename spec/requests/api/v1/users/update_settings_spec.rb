...
RSpec.describe Api::V1::UsersController, type: :request do
    ...
    context 'success' do
      ...
      context "Create one vehicle", :show_in_doc do
        ...
        before do
          ...
          @vehicle_attributes['registration_card'] = fixture_base64_file_upload('spec/files/test.jpg')
        end

        subject do
          put '/api/v1/users/update_settings', headers: { Authorization: get_auth_token(user) },
            params: { user: new_params.merge(vehicles_attributes: [@vehicle_attributes]) }
        end
        ...
        it 'should update user fields and add a vehicle' do
          ...
          expect(vehicle_created.plate_number).to eq(vehicle.plate_number&.remove_non_alphanumeric&.downcase)
      ...
      context "Create a new vehicle and update a vehicle" do
        ...
        before do
          ...
          @vehicle_attributes['registration_card'] = fixture_base64_file_upload('spec/files/test.jpg')
          @new_vehicle_attributes = build(:vehicle).attributes
          @new_vehicle_attributes['registration_card'] = fixture_base64_file_upload('spec/files/test.jpg')
        end

        subject do
          put '/api/v1/users/update_settings', headers: { Authorization: get_auth_token(user) },
            params: { user: new_params.merge(vehicles_attributes: [@new_vehicle_attributes, vehicle_attributes]) }
        end

        it 'should update plate number by creating a new vehicle entity', :show_in_doc do
          new_plate_number = Faker::Car.number
          vehicle_attributes['plate_number'] = new_plate_number
          expect { subject }.to change(Vehicle, :count).by(2)
          user.reload
          vehicle_deleted = user.vehicles.find_by(id: vehicle_attributes['id'])
          vehicle_updated = user.vehicles.find_by(plate_number: new_plate_number&.remove_non_alphanumeric&.downcase)
          expect(vehicle_deleted).to eq(nil)
          expect(vehicle_updated.plate_number).to eq(new_plate_number&.remove_non_alphanumeric&.downcase)
        ...
        it 'should update all attributes of a vehicle' do
          ...
          vehicle_updated = user.vehicles.find_by(plate_number: new_plate_number&.remove_non_alphanumeric&.downcase)
          expect(vehicle_deleted).to eq(nil)
          expect(vehicle_updated.plate_number).to eq(vehicle_attributes['plate_number']&.remove_non_alphanumeric&.downcase)
          expect(vehicle_updated.color).to eq(vehicle_attributes['color'])
          expect(vehicle_updated.vehicle_type).to eq(vehicle_attributes['vehicle_type'])
          expect(vehicle_updated.manufacturer_id).to eq(vehicle_attributes['manufacturer_id'])
          expect(vehicle_updated.model).to eq(vehicle_attributes['model'])
        ...
        context 'when vehicle has parking sessions' do
          context 'with pending status' do
            before do
              @vehicle = create(:vehicle, user_id: user.id, status: 'pending')

              @vehicle_attributes = @vehicle.attributes
              @vehicle_registration_card_url = url_for(@vehicle.registration_card)
              @vehicle_attributes['registration_card'] = fixture_base64_file_upload('spec/files/test.jpg')
              @new_vehicle_attributes =  { id: @vehicle.id, plate_number: @vehicle.plate_number, model: 'CLA', manufacturer_id: @vehicle.manufacturer_id, color: 'White' }
              @new_vehicle_attributes['registration_card'] = fixture_base64_file_upload('spec/files/registration_card.png')
              subject
              @vehicle.reload
            end
            ...
            it 'updates vehicle and set pending status' do
              expect(json['vehicles_attributes'].last[:model]).to eq('CLA')
              expect(json['vehicles_attributes'].last[:color]).to eq('White')
              expect(@vehicle.status).to eq('pending')
              expect(@vehicle_registration_card_url).not_to eq(url_for(@vehicle.registration_card))
            end
          end
        end
      end
      ...
      context "delete vehicles"  do
        let!(:vehicle) { create(:vehicle, user_id: user.id, status: :active) }
        let!(:new_vehicle) { build(:vehicle, user_id: user.id) }
        let(:ai_status) { :exited }
        let(:parking_session_status) { :finished }

        context "Create a new vehicle and delete an old vehicle" do
          before do
            @new_vehicle_attributes = new_vehicle.attributes
            @new_vehicle_attributes['registration_card'] = fixture_base64_file_upload('spec/files/test.jpg')
          end

          subject do
            put '/api/v1/users/update_settings', headers: { Authorization: get_auth_token(user) },
              params: { user: new_params.merge(vehicles_attributes: [@new_vehicle_attributes]) }
          end
        ...
        context "delete all vehicles" do
          let!(:vehicle2) { create(:vehicle, user_id: user.id, status: 'active' ) }
          let!(:vehicle3) { create(:vehicle, user_id: user.id, status: 'rejected') }
          ...
          it 'should update user fields and vehicle' do
            subject
            user.reload
            expect(user.vehicles.size).to eq(0)
          end

        end

        context 'when vehicle has finished parking sessions' do
          let(:vehicle) { create(:vehicle, status: :active, user_id: user.id) }
          ...
          context 'with active status' do
            ...
            it 'unlinks vehicle from user accouct and updates status to inactive' do
              expect(user.vehicles.size).to eq(0)
        ...
        context 'when vehicle has active parking session' do
          let(:vehicle) { create(:vehicle, status: :active, user_id: user.id) }
          ...
          context 'with active status' do
            before do
              create(:parking_session, vehicle: vehicle, ai_status: :entered, status: :created)
              subject
              user.reload
              vehicle.reload
            end
  ...
  context 'fail' do
    ...
    context 'can not update vehicle' do
      context 'when vehicle has parking sessions' do
        context 'with active status' do
          let(:vehicle_attributes) { create(:vehicle, user_id: user.id ).attributes }

          before do
            @vehicle = create(:vehicle, user_id: user.id, status: 'active')
            create(:parking_session, vehicle: @vehicle)
            @vehicle_attributes = @vehicle.attributes
            @vehicle_attributes['registration_card'] = fixture_base64_file_upload('spec/files/test.jpg')
            @new_vehicle_attributes =  { id: @vehicle.id, plate_number: @vehicle.plate_number, model: 'CLA', manufacturer_id: @vehicle.manufacturer_id }
            @new_vehicle_attributes['registration_card'] = fixture_base64_file_upload('spec/files/test.jpg')
            subject
          end
          ...
          it 'returns error message' do
            expect(json[:errors][:base].present?).to be true
          end
        end
      end
    end
