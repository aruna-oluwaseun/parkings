    ...
    module Reports
      module Detailed
        class CitationTicketsController < ApplicationController
          api :GET, '/api/dashboard/reports/detailed/citation_tickets', 'Citation Tickets detailed report'
          header :Authorization, 'Auth token', required: true
          param :citation_ticket_status, String, 'Citation Ticket status'
          ...
          def index
            if params[:citation_ticket_status].present?
              payload = ::Reports::Detailed::CitationTicketsByStatus.run(params.merge(current_user: current_user))
            else
              payload = ::Reports::Detailed::CitationTickets.run(params.merge(current_user: current_user))
            end
